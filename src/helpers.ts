...
export function checkNsInstance(ns: NS, fnName = "this function") { if (!ns.print) throw `The first argument to ${fnName} should be a 'ns' instance.`; return ns; }
 ...
 export function getConfiguration(ns: NS, argsSchema: Array<[string, string | number | boolean | string[]]>) {
     ...
     if (overrides) {
         try {
             ...
             log(ns, `ERROR: There's something wrong with your config file "${confName}", it cannot be loaded.` +
                 `\nThe error encountered was: ${(typeof err === 'string' ? err : (err as Error).message || JSON.stringify(err))}` +
     ...
     try {
         ...
         return finalOptions;
     } catch (err: any) { // Detect if the user passed invalid arguments, and return help text
