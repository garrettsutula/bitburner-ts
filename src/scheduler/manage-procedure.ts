...
export async function main(ns : NS) : Promise<void> {
  ...
  for (const step of steps) {
    ...
    if (steps[index + 1]) {
      ...
      const newProcesses = scheduleAcrossHosts(ns, [], step.script, step.threadsNeeded, host, processId);
      // Send new processes spawned back to scheduler.
      const timeStarted = Date.now();
      await writePortJson(ns, 8, { host, processId, processes: newProcesses, timeStarted });
