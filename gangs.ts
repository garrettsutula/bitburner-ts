...
let allTaskNames;
let allTaskStats;
const assignedTasks = {}; // Each member will independently attempt to scale up the crime they perform until they are ineffective or we start generating wanted levels
const lastMemberReset = {}; // Tracks when each member last ascended
...
export function autocomplete(data: any, _) {
...
export async function main(ns: NS) {
    ...
    await initialize(ns);
    ns.print(ns, "Starting main loop...");
    while (true) {
...
async function optimizeGangCrime(ns, myGangInfo) {
    ...
    const wantedGainTolerance = currentWantedPenalty < -1.1 * wantedPenaltyThreshold && myGangInfo.wantedLevel >= (1.1 + myGangInfo.respect / 1000) &&
...
async function tryUpgradeMembers(ns, dictMembers) {
    ...
    for (const equip of equipments) {
        for (const member of Object.values(dictMembers)) { // Get this equip for each member before considering the next most expensive equip
            ...
            const percievedCost = equip.cost * (Object.keys(equip.stats).some(stat => importantStats.some(i => stat.includes(i))) ? 1 : offStatCostPenalty);
...
const getStatWeight = (task, memberInfo) =>
...
const getWantedPenalty = myGangInfo => myGangInfo.respect / (myGangInfo.respect + myGangInfo.wantedLevel);
const getTerritoryPenalty = myGangInfo => (0.2 * myGangInfo.territory + 0.8) * multGangSoftcap;
...
/** @param {NS} ns **/
function log(ns: NS, message: string, toastStyle?: string, terminal = undefined) {
    ns.print(message);
